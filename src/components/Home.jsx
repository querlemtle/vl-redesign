...
const decos = [
  { alt: "愛心彩帶", styleTag: deco1 },
  { alt: "圓形碎紙", styleTag: deco2 },
  { alt: "方塊碎紙", styleTag: deco3 },
  { alt: "圓圈彩帶", styleTag: deco4 },
  { alt: "方塊碎紙", styleTag: deco5 },
  { alt: "波浪形彩帶", styleTag: deco6 },
  { alt: "圓形碎紙", styleTag: deco7 },
  { alt: "愛心彩帶", styleTag: deco8 },
];

export default function Home({ newsData, productsData }) {
    ...
    <>
      ...
      <section className={section} ref={gsapContainer}>
        <div className={twoTonesBg}>
          {decos.map((item, i) => (
            <img
              key={i}
              src={`/icons/confetti-${i + 1}.svg`}
              className={`${deco} ${item.styleTag}`}
              alt={item.alt}
              data-ani="spread-out"
            />
          ))}
      ...
      <section className={`${section} ${sectionNews}`}>
        ...
        <div className="news__grid">
          ...
          <Link href="/news?page=1" className={`${btn} ${btnsContainer}`}>
            VIEW MORE{" "}
            <img
